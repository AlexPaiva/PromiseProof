...
async function copyRuntimeControlAcls(input: {
  ...
  const script = [
    ...
    `$sourceMarkerAcl = Get-Acl -LiteralPath ${powerShellLiteral(input.sourceMarkerPath)}`,
    `$sourceRunnerAcl = Get-Acl -LiteralPath ${powerShellLiteral(input.sourceRunnerPath)}`,
    '$sourceMarkerAcl.SetAccessRuleProtection($true, $true)',
    '$sourceRunnerAcl.SetAccessRuleProtection($true, $true)',
    `Set-Acl -LiteralPath ${powerShellLiteral(input.copiedMarkerPath)} -AclObject $sourceMarkerAcl`,
    `Set-Acl -LiteralPath ${powerShellLiteral(input.copiedRunnerPath)} -AclObject $sourceRunnerAcl`,
    `$copiedMarkerAcl = Get-Acl -LiteralPath ${powerShellLiteral(input.copiedMarkerPath)}`,
    `$copiedRunnerAcl = Get-Acl -LiteralPath ${powerShellLiteral(input.copiedRunnerPath)}`,
