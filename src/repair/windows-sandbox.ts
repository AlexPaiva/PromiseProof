...
async function copyRuntimeControlAcls(input: {
  ...
  const script = [
    ...
    'if ($sourceMarkerAcl.GetOwner([System.Security.Principal.SecurityIdentifier]).Value -ne $copiedMarkerAcl.GetOwner([System.Security.Principal.SecurityIdentifier]).Value) { exit 53 }',
    'if ($sourceRunnerAcl.GetOwner([System.Security.Principal.SecurityIdentifier]).Value -ne $copiedRunnerAcl.GetOwner([System.Security.Principal.SecurityIdentifier]).Value) { exit 54 }',
    '$accessSection = [System.Security.AccessControl.AccessControlSections]::Access',
    '$copiedMarkerAcl.SetSecurityDescriptorSddlForm($sourceMarkerAcl.GetSecurityDescriptorSddlForm($accessSection), $accessSection)',
    '$copiedRunnerAcl.SetSecurityDescriptorSddlForm($sourceRunnerAcl.GetSecurityDescriptorSddlForm($accessSection), $accessSection)',
    `[System.IO.File]::SetAccessControl(${powerShellLiteral(input.copiedMarkerPath)}, $copiedMarkerAcl)`,
    `[System.IO.File]::SetAccessControl(${powerShellLiteral(input.copiedRunnerPath)}, $copiedRunnerAcl)`,
    `$copiedMarkerAcl = [System.IO.File]::GetAccessControl(${powerShellLiteral(input.copiedMarkerPath)})`,
    `$copiedRunnerAcl = [System.IO.File]::GetAccessControl(${powerShellLiteral(input.copiedRunnerPath)})`,
