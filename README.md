...
Milestone 04 now includes live-receipt eligibility, a pinned one-turn Codex provider, two disposable worktrees, an elevated Windows offline-user sandbox, semantic source/test firewalls, digest-bound real-TTY review, deterministic fresh-worktree verification, explicit `not_run` retirement, crash recovery, and tamper-evident receipts. The final correction is committed and pushed after an app-owned transient ref rotation exposed an over-strict cross-invocation comparison. Post-hardening checks pass: the repair unit boundary is 131/131, the ordinary race and propagation browser suites are 6/6 each, deterministic contexts are 20/20, both expected-red verifiers retain only their intended code, the elevated sandbox is 1/1, and the clean committed offline two-worktree journey is 1/1. The old approved candidate has now been retired through Alex's real TTY with `verificationVerdict: not_run` and `cleanup_completed`; it was never a production repaired-state PASS. One fresh candidate must be prepared from the corrected base.

The canonical evaluator, contract assertion, seeded defects, and evidence capture remain outside model authority and must stay unchanged. The repository is not yet a complete Build Week submission. The immediate gates are one fresh authentic candidate, human review, and a fresh-worktree Playwright PASS. Cross-platform CI, a no-rebuild judge path, licensing, hosting, narrated demo packaging, screenshots, and the Devpost submission remain subsequent readiness work.
